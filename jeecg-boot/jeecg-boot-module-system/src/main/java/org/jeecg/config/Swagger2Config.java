...
//				//.globalOperationParameters(setHeaderToken());
				.apiInfo(apiInfo()).groupName("系统System")
				.select()
				//此包路径下的类，才生成接口文档
...
//				.securitySchemes(Collections.singletonList(securityScheme()));
//				//.globalOperationParameters(setHeaderToken());
				.apiInfo(apiInfo()).groupName("模块Hello")
				.select()
				//此包路径下的类，才生成接口文档
	...
	@Bean
	public Docket createTemRestApi() {
		return new Docket(DocumentationType.SWAGGER_2)
//				.apiInfo(apiInfo())
//				.select()
//				//此包路径下的类，才生成接口文档
...
//				.securitySchemes(Collections.singletonList(securityScheme()));
//				//.globalOperationParameters(setHeaderToken());
//				.apiInfo(apiInfo()).groupName("模块sale")
				//.apiInfo(apiInfo())
				.apiInfo(apiInfo()).groupName("模块sale")
				.select()
				//此包路径下的类，才生成接口文档
				//.apis(RequestHandlerSelectors.basePackage("com.tempal.modules"))
				.apis(RequestHandlerSelectors.basePackage("org.jeecg.modules"))
				//加了ApiOperation注解的类，才生成接口文档
				.apis(RequestHandlerSelectors.withMethodAnnotation(ApiOperation.class))
				.paths(PathSelectors.any())
